...
def detect_anomalies(
    ...
    cfg = {**DEFAULTS, **(cfg or {})}
    anomalies: list[dict] = []
    for vendor, cur in sorted(fresh.items()):
...
def update_baseline(
    ...
    """Return a NEW baseline. For a vendor healthy this run:
      - not anomalous → EWMA-update count/stock, bump samples, reset streak.
      - anomalous     → leave count/stock put (no boiling-frog), bump streak.
